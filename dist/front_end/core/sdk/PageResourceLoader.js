...
var _PageResourceLoader_currentlyLoading, _PageResourceLoader_currentlyLoadingPerTarget, _PageResourceLoader_maxConcurrentLoads, _PageResourceLoader_pageResources, _PageResourceLoader_queuedLoads, _PageResourceLoader_loadOverride;
...
export class PageResourceLoader extends Common.ObjectWrapper.ObjectWrapper {
    constructor(loadOverride, maxConcurrentLoads) {
        super();
        _PageResourceLoader_currentlyLoading.set(this, 0);
        _PageResourceLoader_currentlyLoadingPerTarget.set(this, new Map());
        _PageResourceLoader_maxConcurrentLoads.set(this, void 0);
        _PageResourceLoader_pageResources.set(this, new Map());
        _PageResourceLoader_queuedLoads.set(this, []);
        _PageResourceLoader_loadOverride.set(this, void 0);
        __classPrivateFieldSet(this, _PageResourceLoader_maxConcurrentLoads, maxConcurrentLoads, "f");
        ...
        __classPrivateFieldSet(this, _PageResourceLoader_loadOverride, loadOverride, "f");
    }
    ...
    onPrimaryPageChanged(event) {
        ...
        for (const { reject } of __classPrivateFieldGet(this, _PageResourceLoader_queuedLoads, "f")) {
            reject(new Error(i18nString(UIStrings.loadCanceledDueToReloadOf)));
        }
        __classPrivateFieldSet(this, _PageResourceLoader_queuedLoads, [], "f");
        ...
        for (const [key, pageResource] of __classPrivateFieldGet(this, _PageResourceLoader_pageResources, "f").entries()) {
        ...
        __classPrivateFieldSet(this, _PageResourceLoader_pageResources, keptResources, "f");
        this.dispatchEventToListeners("Update" /* Events.UPDATE */);
    }
    getResourcesLoaded() {
        return __classPrivateFieldGet(this, _PageResourceLoader_pageResources, "f");
    }
    getScopedResourcesLoaded() {
        return new Map([...__classPrivateFieldGet(this, _PageResourceLoader_pageResources, "f")].filter(([_, pageResource]) => TargetManager.instance().isInScope(pageResource.initiator.target) ||
    ...
    getNumberOfResources() {
        return { loading: __classPrivateFieldGet(this, _PageResourceLoader_currentlyLoading, "f"), queued: __classPrivateFieldGet(this, _PageResourceLoader_queuedLoads, "f").length, resources: __classPrivateFieldGet(this, _PageResourceLoader_pageResources, "f").size };
    }
    getScopedNumberOfResources() {
        const targetManager = TargetManager.instance();
        let loadingCount = 0;
        for (const [targetId, count] of __classPrivateFieldGet(this, _PageResourceLoader_currentlyLoadingPerTarget, "f")) {
    ...
    async acquireLoadSlot(target) {
        var _a;
        __classPrivateFieldSet(this, _PageResourceLoader_currentlyLoading, (_a = __classPrivateFieldGet(this, _PageResourceLoader_currentlyLoading, "f"), _a++, _a), "f");
        if (target) {
            const currentCount = __classPrivateFieldGet(this, _PageResourceLoader_currentlyLoadingPerTarget, "f").get(target.id()) || 0;
            __classPrivateFieldGet(this, _PageResourceLoader_currentlyLoadingPerTarget, "f").set(target.id(), currentCount + 1);
        }
        if (__classPrivateFieldGet(this, _PageResourceLoader_currentlyLoading, "f") > __classPrivateFieldGet(this, _PageResourceLoader_maxConcurrentLoads, "f")) {
            const { promise: waitForCapacity, resolve, reject, } = Promise.withResolvers();
            __classPrivateFieldGet(this, _PageResourceLoader_queuedLoads, "f").push({ resolve, reject });
    ...
    releaseLoadSlot(target) {
        ...
        __classPrivateFieldSet(this, _PageResourceLoader_currentlyLoading, (_a = __classPrivateFieldGet(this, _PageResourceLoader_currentlyLoading, "f"), _a--, _a), "f");
        if (target) {
            const currentCount = __classPrivateFieldGet(this, _PageResourceLoader_currentlyLoadingPerTarget, "f").get(target.id());
            if (currentCount) {
                __classPrivateFieldGet(this, _PageResourceLoader_currentlyLoadingPerTarget, "f").set(target.id(), currentCount - 1);
            }
        }
        const entry = __classPrivateFieldGet(this, _PageResourceLoader_queuedLoads, "f").shift();
        if (entry) {
            entry.resolve();
        }
    }
    ...
    resourceLoadedThroughExtension(pageResource) {
        ...
        __classPrivateFieldGet(this, _PageResourceLoader_pageResources, "f").set(key, pageResource);
        this.dispatchEventToListeners("Update" /* Events.UPDATE */);
    }
    ...
    async dispatchLoad(url, initiator, isBinary) {
        ...
        if (__classPrivateFieldGet(this, _PageResourceLoader_loadOverride, "f")) {
            return await __classPrivateFieldGet(this, _PageResourceLoader_loadOverride, "f").call(this, url);
        }
...
_PageResourceLoader_currentlyLoading = new WeakMap(), _PageResourceLoader_currentlyLoadingPerTarget = new WeakMap(), _PageResourceLoader_maxConcurrentLoads = new WeakMap(), _PageResourceLoader_pageResources = new WeakMap(), _PageResourceLoader_queuedLoads = new WeakMap(), _PageResourceLoader_loadOverride = new WeakMap();
