...
export class VisionAgentEvaluationRunner {
    constructor(visionEnabled = false, judgeModel) {
     ...
     */
    async runSingleTest(testCase, agentName) {
        ...
        try {
            ...
            if (isTracingEnabled()) {
                ...
                await this.tracingProvider.createTrace(traceId, tracingContext.sessionId, `Vision Agent Evaluation: ${testCase.name}`, testCase.input, {
                    testId: testCase.id,
                ...
                }, 'vision-agent-runner', ['evaluation', 'vision', toolName]);
            }
        }
        catch (error) {
        ...
        try {
            ...
            const agentResult = await evaluator.runTest(testCase, agent, tracingContext);
            ...
            if (agentResult.status === 'passed' && agentResult.output && testCase.validation.type === 'llm-judge') {
                ...
                const enhancedValidation = this.createEnhancedValidationConfig(agentResult.output, testCase.validation);
                if (shouldUseVision && (beforeScreenshot || afterScreenshot)) {
                    ...
                    llmJudgment = await this.llmEvaluator.evaluate(agentResult.output, testCase, enhancedValidation, { before: beforeScreenshot, after: afterScreenshot });
                }
                else {
                    logger.info('📝 Running standard LLM evaluation...');
                    // Use unified LLM evaluator without vision
                    llmJudgment = await this.llmEvaluator.evaluate(agentResult.output, testCase, enhancedValidation);
                }
     ...
     * Create enhanced validation config with agent-specific criteria
     */
    createEnhancedValidationConfig(output, inputValidationConfig) {
        ...
        if (conversationInfo.toolsUsed.length > 0) {
            enhancedCriteria.push(`Agent effectively utilized available tools: ${conversationInfo.toolsUsed.join(', ')}`);
        }
     ...
     */
    extractConversationInfo(output) {
        const info = {
            toolsUsed: [],
            stepCount: 0,
     ...
     */
    static isVisionEnabled(testCase) {
...
//# sourceMappingURL=VisionAgentEvaluationRunner.js.map