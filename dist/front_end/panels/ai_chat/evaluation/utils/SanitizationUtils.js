...
export class SanitizationUtils {
     ...
     */
    static isDynamicField(key, value) {
        ...
        if (lowerKey.includes('x') || lowerKey.includes('y'))
            return true;
        if (lowerKey.includes('top') || lowerKey.includes('left'))
            return true;
     ...
     */
    static sanitizeDynamicValue(key, _value) {
        const lowerKey = key.toLowerCase();
        if (lowerKey.includes('timestamp') || lowerKey.includes('time')) {
            return '[TIMESTAMP]';
        }
...
//# sourceMappingURL=SanitizationUtils.js.map