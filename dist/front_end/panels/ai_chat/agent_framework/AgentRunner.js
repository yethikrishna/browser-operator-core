...
export class AgentRunner {
     ...
     */
    static addSessionMessage(message) {
    ...
    static async executeHandoff(currentMessages, originalArgs, handoffConfig, executingAgent, apiKey, defaultModelName, defaultMaxIterations, defaultTemperature, defaultCreateSuccessResult, defaultCreateErrorResult, llmToolArgs, // Specific args if triggered by LLM tool call
    parentSession // For natural nesting
    ) {
        ...
        let handoffMessages = []; // Initialize handoff messages
        ...
        const handoffResult = await AgentRunner.run(handoffMessages, targetAgentArgs, // Use determined args
    ...
    static async run(initialMessages, args, config, hooks, executingAgent, parentSession // For natural nesting
    ) {
...
- Focus on making meaningful progress with each step.`;
            ...
            let llmResponse;
            let generationId; // Declare in iteration scope for tool call access
            try {
            ...
            try {
                let newModelMessage;
                let toolCallObservationId; // Declare in action scope for tool execution access
                if (parsedAction.type === 'tool_call') {
                    ...
                    if (toolName.startsWith('handoff_to_') && toolToExecute instanceof ConfigurableAgentTool) {
                        ...
                        const handoffConfig = executingAgent?.config.handoffs?.find(h => h.targetAgentName === targetAgentTool.name &&
                        ...
                        const handoffResult = await AgentRunner.executeHandoff(messages, // Pass current message history
                        toolArgs, // <= LLM's toolArgs are the 'originalArgs' for this handoff context
                        handoffConfig, executingAgent, // executingAgent must exist if handoff config was found
                        apiKey, modelName, maxIterations, temperature ?? 0, createSuccessResult, createErrorResult, toolArgs, // <= Pass LLM's toolArgs explicitly as llmToolArgs
        ...
        if (executingAgent?.config.handoffs) {
            ...
            if (maxIterHandoffConfig) {
                ...
                const handoffResult = await AgentRunner.executeHandoff(messages, // Pass the final message history from the loop
                args, // Pass the original args of the *current* agent as 'originalArgs'
                maxIterHandoffConfig, executingAgent, apiKey, modelName, maxIterations, temperature ?? 0, createSuccessResult, createErrorResult, undefined, // No llmToolArgs for max iterations handoff
                this.currentSession // Pass current session for natural nesting
                );
                // Extract the result and session
                const { agentSession: childSession, ...actualResult } = handoffResult;
     ...
     */
    static async summarizeAgentProgress(messages, maxIterations, agentName, modelName, completionType = 'max_iterations') {
...
AgentRunner.currentSession = null;
