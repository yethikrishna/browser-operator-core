...
export class OpenRouterProvider extends LLMBaseProvider {
    ...
    modelSupportsReasoning(model) {
        ...
        return reasoningModels.some(modelType => model.id.toLowerCase().includes(modelType) ||
    ...
    modelSupportsVision(model) {
        ...
        return visionModels.some(modelType => model.id.toLowerCase().includes(modelType) ||
            model.name?.toLowerCase().includes(modelType));
    }
    /**
...
OpenRouterProvider.API_BASE_URL = 'https://openrouter.ai/api/v1';
OpenRouterProvider.CHAT_COMPLETIONS_PATH = '/chat/completions';
OpenRouterProvider.MODELS_PATH = '/models';
OpenRouterProvider.CACHE_DURATION_MS = 30 * 60 * 1000; // 30 minutes
