...
export function createAgentNode(modelName, temperature) {
    const agentNode = new class AgentNode {
        constructor(modelName, temperature) {
            this.callCount = 0;
            this.MAX_CALLS_PER_INTERACTION = 50;
        ...
        async invoke(state) {
            ...
            logger.debug('AgentNode: Invoked with state. Last message:', state.messages.length > 0 ? state.messages[state.messages.length - 1] : 'No messages');
         ...
         */
        sanitizeToolResultForText(toolResultData) {
         ...
         * Convert ChatMessage[] to LLMMessage[]
         */
        convertChatMessagesToLLMMessages(messages) {
            const llmMessages = [];
...
export function createToolExecutorNode(state) {
    ...
    const toolExecutorNode = new class ToolExecutorNode {
        constructor(toolMap) {
        ...
        async invoke(state) {
            ...
            try {
                ...
                const result = await withTracingContext(executionContext, async () => {
                    ...
                    return await selectedTool.execute(toolArgs);
                });
                ...
                if (selectedTool instanceof ConfigurableAgentTool && result && typeof result === 'object' && 'agentSession' in result) {
                    ...
                    if (agentSession.messages) {
                        const toolCalls = agentSession.messages.filter((msg) => msg.type === 'tool_call');
                        const toolResults = agentSession.messages.filter((msg) => msg.type === 'tool_result');
                        console.log(`[AGENT SESSION] Tool Analysis:`, {
                            ...
                            messageTypes: agentSession.messages.map((msg) => ({ type: msg.type, id: msg.id }))
                        });
                    }
                    ...
                    console.log(`[AGENT SESSION] Created AgentSessionMessage:`, {
                        sessionId: result.agentSession.sessionId,
                        agentName: result.agentSession.agentName,
                        status: result.agentSession.status
                    });
                ...
                if (selectedTool instanceof ConfigurableAgentTool && result && typeof result === 'object' && 'output' in result) {
                    ...
                    const agentResult = result; // Cast to any to access ConfigurableAgentResult properties
...
export function createFinalNode() {
    const finalNode = new class FinalNode {
        async invoke(state) {
            const lastMessage = state.messages[state.messages.length - 1];
...
//# sourceMappingURL=AgentNodes.js.map