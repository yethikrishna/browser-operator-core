...
export class PageInfoManager {
    ...
    constructor() {
        ...
        SDK.TargetManager.TargetManager.instance().addEventListener("InspectedURLChanged" /* SDK.TargetManager.Events.INSPECTED_URL_CHANGED */, () => this.updatePageInfo());
     ...
     */
    async updatePageInfoWithFullTree() {
    ...
    async fetchAccessibilityTree(target) {
        try {
 ...
 */
export async function enhancePromptWithPageContext(basePrompt) {
...
<Context>
  ...
  <Page>
    ...
    ${iframeContent && iframeContent.length > 0 ?
        `<Iframes>
      ${iframeContent.map((iframe, index) => `<Iframe index="${index + 1}" role="${iframe.role}"${iframe.name ? ` name="${iframe.name}"` : ''}>
          <Content>
...
//# sourceMappingURL=PageInfoManager.js.map