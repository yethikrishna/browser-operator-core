...
export class StreamlinedSchemaExtractorTool {
    constructor() {
        this.name = 'extract_schema_streamlined';
        this.description = `Tool for extracting structured data from web pages using JSON schema.
  - Returns: { success, data, error (if any) }`;
        this.MAX_URL_RETRIES = 4;
        this.MAX_JSON_RETRIES = 2;
        this.RETRY_DELAY_MS = 10000; // 10 second delay between retries
        this.schema = {
            type: 'object',
            properties: {
                schema: {
    ...
    async execute(args) {
        try {
            ...
            const extractionResult = await this.performExtraction(context);
            const finalData = await this.resolveUrlsWithRetry(extractionResult, context);
            return {
                success: true,
                data: finalData
            };
    ...
    async getAccessibilityData(target) {
    ...
    async performExtraction(context) {
        return await this.extractWithJsonRetry(context.schema, context.treeText, context.instruction, context.apiKey, this.MAX_JSON_RETRIES);
    }
    async resolveUrlsWithRetry(extractionResult, context) {
        ...
        for (let attempt = 1; attempt <= this.MAX_URL_RETRIES; attempt++) {
            ...
            const retryResult = await this.retryUrlResolution(context.schema, context.treeText, context.instruction, extractionResult, unresolvedNodeIds, context.apiKey, attempt);
            if (retryResult) {
    ...
    async extractWithJsonRetry(schema, treeText, instruction, apiKey, maxRetries) {
...
IMPORTANT: Only extract data that you can see in the accessibility tree above. Do not make up, guess, or hallucinate any values.`;
    ...
    findUnresolvedNodeIds(data, urlFields) {
    ...
    async retryUrlResolution(schema, treeText, instruction, originalResult, unresolvedNodeIds, apiKey, attemptNumber) {
...
CRITICAL: Only use nodeIds that you can actually see in the accessibility tree above. Do not invent, guess, or make up any nodeIds.`;
     ...
     */
    findUrlFields(schema) {
        const urlFields = [];
        const findInProperties = (properties, prefix = '') => {
     ...
     */
    resolveUrlsDirectly(data, urlMappings, urlFields) {
...
//# sourceMappingURL=StreamlinedSchemaExtractorTool.js.map