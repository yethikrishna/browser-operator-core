...
// Type imports
import { CombinedExtractionTool } from './CombinedExtractionTool.js';
import { FetcherTool } from './FetcherTool.js';
import { FinalizeWithCritiqueTool } from './FinalizeWithCritiqueTool.js';
import { FullPageAccessibilityTreeToMarkdownTool } from './FullPageAccessibilityTreeToMarkdownTool.js';
import { HTMLToMarkdownTool } from './HTMLToMarkdownTool.js';
import { SchemaBasedExtractorTool } from './SchemaBasedExtractorTool.js';
import { VisitHistoryManager } from './VisitHistoryManager.js';
...
export class ExecuteJavaScriptTool {
    constructor() {
        this.name = 'execute_javascript';
        this.description = 'Executes JavaScript code in the page context';
        this.schema = {
            type: 'object',
            properties: {
                code: {
 ...
 */
export class NetworkAnalysisTool {
    constructor() {
        this.name = 'analyze_network';
        this.description = 'Analyzes network requests, optionally filtered by URL pattern';
        this.schema = {
            type: 'object',
            properties: {
                url: {
    ...
    async execute(args) {
        ...
        try {
            ...
            const mappedRequests = await Promise.all(limitedRequests.map(async (request) => {
 ...
 */
export async function waitForPageLoad(target, timeoutMs) {
    ...
    try {
      ...
      `;
            try {
                ...
                const lcpStatus = result.result.value;
...
export class NavigateURLTool {
    constructor() {
        this.name = 'navigate_url';
        this.description = 'Navigates the page to a specified URL and waits for it to load';
        this.schema = {
            type: 'object',
            properties: {
                url: {
    ...
    async check404Status(target, metadata) {
        try {
    ...
    async confirmWith404LLM(url, title, content) {
        try {
 ...
 */
export class NavigateBackTool {
    constructor() {
        this.name = 'navigate_back';
        this.description = 'Navigates back in browser history by a specified number of steps';
        this.schema = {
            type: 'object',
            properties: {
                steps: {
 ...
 */
export class GetPageHTMLTool {
    constructor() {
        this.name = 'get_page_html';
        this.description = 'Gets the HTML contents and structure of the current page for analysis and summarization with CSS, JavaScript, and other non-essential content removed';
        this.schema = {
            type: 'object',
            properties: {},
        };
...
export class ClickElementTool {
    constructor() {
        ...
        this.description = 'Clicks on an element identified by a CSS selector';
        this.schema = {
            type: 'object',
            properties: {
                selector: {
...
export class SearchContentTool {
    constructor() {
        ...
        this.description = 'Searches for text content on the page and returns matching elements';
        this.schema = {
            type: 'object',
            properties: {
                query: {
...
export class ScrollPageTool {
    constructor() {
        this.name = 'scroll_page';
        this.description = 'Scrolls the page to a specific position or in a specific direction';
        this.schema = {
            type: 'object',
            properties: {
                position: {
...
export class WaitTool {
    constructor() {
        this.name = 'wait_for_page_load';
        this.description = 'Waits for a specified number of seconds to allow page content to load, animations to complete, or dynamic content to appear. After waiting, returns a summary of what is currently visible in the viewport to help determine if additional waiting is needed. Provide the number of seconds to wait and an optional reasoning for waiting.';
        this.schema = {
            type: 'object',
            properties: {
                seconds: {
...
export class TakeScreenshotTool {
    constructor() {
        ...
        this.description = 'Takes a screenshot of the current page view or the entire page. The image can be used for analyzing the page layout, content, and visual elements. Always specify whether to capture the full page or just the viewport and the reasoning behind it.';
        this.schema = {
            type: 'object',
            properties: {
                fullPage: {
 ...
 */
export class GetAccessibilityTreeTool {
    constructor() {
        this.name = 'get_page_content';
        this.description = 'Gets the accessibility tree of the current page, providing a hierarchical structure of all accessible elements.';
        this.schema = {
            type: 'object',
            properties: {
                reasoning: {
 ...
 */
export class GetVisibleAccessibilityTreeTool {
    constructor() {
        this.name = 'get_visible_content';
        this.description = 'Gets the accessibility tree of only the visible content in the viewport, providing a focused view of what the user can currently see.';
        this.schema = {
            type: 'object',
            properties: {
                reasoning: {
 ...
 */
export class PerformActionTool {
    constructor() {
        this.name = 'perform_action';
        this.description = 'Performs an action on a DOM element identified by NodeID';
        this.schema = {
            type: 'object',
            properties: {
                method: {
    ...
    async execute(args) {
        ...
        let iframeNodeId;
        let elementNodeId;
        let treeResult = null; // Cache the tree result to avoid multiple calls
        try {
            ...
            if (isIframeNodeId) {
                ...
                if (treeResult.xpathMap && treeResult.xpathMap[nodeId]) {
                    xpath = treeResult.xpathMap[nodeId];
                    logger.info('Found XPath from xpathMap:', xpath);
                }
                else {
                    // Fallback to CDP call
                    xpath = await Utils.getXPathByBackendNodeId(target, nodeId);
                    if (!xpath || xpath === '') {
            ...
            if (method === 'fill' || method === 'type') {
                ...
                if (typeof args.args !== 'object' || args.args === null || Array.isArray(args.args) || typeof args.args.text !== 'string') {
                ...
                const textValue = args.args.text;
                ...
                if (!iframeNodeId) {
                    const treeResult = await Utils.getAccessibilityTree(target);
                    if (treeResult.tagNameMap && treeResult.tagNameMap[nodeId]) {
                        elementTagName = treeResult.tagNameMap[nodeId];
              ...
              return { suitable, reason };
                ...
                const optionValue = args.args.text;
                ...
                if (typeof args.args !== 'object' || args.args === null || Array.isArray(args.args) || typeof args.args.checked !== 'boolean') {
                ...
                const checkedValue = args.args.checked;
            ...
            if (method === 'fill' || method === 'type') {
                ...
                const expectedValue = args.args.text;
                try {
...
Provide a clear, descriptive response about what you observe and whether the action appears to have succeeded.`
    ...
    async waitForDOMStability(target, method, isLikelyNavigationElement) {
    ...
    async waitForDocumentReady(target, maxWaitTime) {
        const startTime = Date.now();
    ...
    async waitForDOMMutationStability(target, maxWaitTime) {
        const startTime = Date.now();
        ...
        while (Date.now() - startTime < maxWaitTime) {
          ...
          `,
                ...
                if (!currentTreeResult.exceptionDetails && currentTreeResult.result.value) {
                    const currentHash = currentTreeResult.result.value;
    ...
    findModifications(before, after, added, removed, modified) {
...
export class ObjectiveDrivenActionTool {
    constructor() {
        this.name = 'objective_driven_action';
        this.description = 'Analyzes the page\'s accessibility tree to fulfill a delegated action objective. Performs actions (e.g., click, fill) using accessibility IDs. Identifies the best element to interact with based on the context and objectives. Acts as a specialized sub-agent with retries.';
        this.schema = {
            type: 'object',
            properties: {
                objective: {
...
- Choose the most semantically appropriate element when multiple options exist.`;
    ...
    async execute(args) {
        ...
        while (currentTry <= maxRetries) {
            ...
            let attemptError = null; // Use Error object for better stack traces
            try {
                // --- Step 1: Get Tree ---
...
- Choose the most semantically appropriate element when multiple options exist.`;
                ...
                const { method: actionMethod, nodeId: accessibilityNodeId, args: actionArgs } = response.functionCall.arguments;
                logger.info('Parsed Tool Arguments:', { actionMethod, accessibilityNodeId, actionArgs });
                const actionNodeId = accessibilityNodeId;
                ...
                if (pageTarget) {
                    ...
                    metadata = metadataEval.result.value;
                }
                return {
                    success: true,
 ...
 */
export class NodeIDsToURLsTool {
    constructor() {
        this.name = 'get_urls_from_nodeids';
        this.description = 'Gets URLs associated with DOM elements identified by NodeIDs from accessibility tree.';
        this.schema = {
            type: 'object',
            properties: {
                nodeIds: {
    ...
    async execute(args) {
        ...
        for (const nodeId of args.nodeIds) {
            try {
                ...
                const xpath = await getXPathByBackendNodeId(target, nodeId);
                if (!xpath) {
                    results.push({ nodeId });
                    continue;
                }
...
4. All nodeId values must be numbers, not strings.`;
     ...
     */
    getAggregatedNodeContent(node, allNodes) {
     ...
     */
    async processNodeStructure(structure, accessibilityNodes = []) {
        ...
        const mappedNodes = accessibilityNodes.map((node) => {
            ...
            const nameValue = node.name && typeof node.name === 'object' && 'value' in node.name
                ...
                : undefined;
            const descriptionValue = node.description &&
     ...
     */
    hasRelevantChildren(node, allNodes) {
    ...
    async processNodeStructureRecursive(structure, accessibilityNodes, mappedNodes) {
...
Extract NodeIDs according to the provided objective and schema, then return a structured JSON with NodeIDs instead of content.`;
                ...
                try {
                    ...
                    throw new Error(`LLM did not return valid JSON data: ${error.message}`);
                }
...
export class GetVisitsByDomainTool {
    constructor() {
        this.name = 'get_visits_by_domain';
        this.description = 'Get a list of visited pages filtered by domain name';
        this.schema = {
            type: 'object',
            properties: {
                domain: {
...
export class GetVisitsByKeywordTool {
    constructor() {
        this.name = 'get_visits_by_keyword';
        this.description = 'Get a list of visited pages containing a specific keyword';
        this.schema = {
            type: 'object',
            properties: {
                keyword: {
...
export class SearchVisitHistoryTool {
    constructor() {
        this.name = 'search_visit_history';
        this.description = 'Search browsing history with multiple filter criteria';
        this.schema = {
            type: 'object',
            properties: {
                domain: {
