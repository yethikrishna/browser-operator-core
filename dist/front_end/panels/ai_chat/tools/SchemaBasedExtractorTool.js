...
import { NodeIDsToURLsTool } from './Tools.js';
 ...
 */
export class SchemaBasedExtractorTool {
    constructor() {
        this.name = 'extract_schema_data';
        this.description = `Extracts structured data from a web page's DOM using a user-provided JSON schema and natural language instruction.
...
• News articles: {"type": "object", "properties": {"articles": {"type": "array", "items": {"type": "object", "properties": {"headline": {"type": "string"}, "author": {"type": "string"}, "publishDate": {"type": "string"}, "link": {"type": "string", "format": "url"}}}}}}`;
    ...
    async execute(args) {
        ...
        try {
            ...
            //    };
            // }
            const rootBackendNodeId = undefined;
            const rootNodeId = undefined;
            ...
            logger.debug('Data after URL resolution:', JSON.stringify(Array.isArray(finalData) ? finalData.slice(0, 2) : finalData, null, 2).substring(0, 500));
            // 7a. Check if any URL fields still contain numeric node IDs
            let urlResolutionWarning;
     ...
     */
    transformUrlFieldsToIds(schema) {
        const urlPaths = [];
     ...
     */
    processSchemaProperties(properties, currentPath, urlPaths) {
        const result = {};
     ...
     */
    buildUrlMapping(nodes) {
        logger.debug(`Building URL mapping from ${nodes.length} nodes`);
        const idToUrlMapping = {};
        for (const node of nodes) {
            const urlProperty = node.properties?.find(p => p.name === "url" /* Protocol.Accessibility.AXPropertyName.Url */);
     ...
     */
    async callExtractionLLM(options) {
...
Only output the JSON object with real data from the accessibility tree.`;
        try {
            ...
            if (!response) {
                throw new Error('No text response from extraction LLM');
            }
     ...
     */
    async callRefinementLLM(options) {
...
Do not add any conversational text or explanations or thinking tags.`;
        try {
            ...
            const response = llmResponse.text;
            if (!response) {
                throw new Error('No text response from refinement LLM');
            }
            return this.parseJsonResponse(response);
        }
        catch (error) {
...
Return ONLY a valid JSON object conforming to the required metadata schema.`;
        try {
            ...
            const response = llmResponse.text;
            if (!response) {
                throw new Error('No text response from metadata LLM');
            }
     ...
     */
    findAndReplaceNodeIds(data, nodeIdToUrlMap) {
     ...
     */
    collectPotentialNodeIds(data, nodeIds) {
        if (data === null || data === undefined) {
            return;
        }
     ...
     */
    async resolveUrlsWithLLM(options) {
...
//# sourceMappingURL=SchemaBasedExtractorTool.js.map