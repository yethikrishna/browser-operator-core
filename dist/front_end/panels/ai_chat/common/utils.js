 ...
 */
function buildSubtreeRecursive(nodeId, nodeMap) {
    ...
    if (currentNode.childIds && currentNode.childIds.length > 0) {
        newNode.children = currentNode.childIds
            ...
            .filter((child) => child !== null); // Filter out null results if a child isn't found
    }
 ...
 */
export function getFormattedSubtreeByNodeId(targetNodeId, allNodes) {
 ...
 */
async function buildBackendIdMaps(target) {
...
function extractUrlFromAXNode(axNode) {
    if (!axNode.properties)
        return undefined;
    const urlProp = axNode.properties.find(prop => prop.name === "url" /* Protocol.Accessibility.AXPropertyName.Url */);
...
function removeRedundantStaticTextChildren(parent, children) {
    ...
    const staticTextChildren = children.filter(child => child.role === 'StaticText' && child.name);
 ...
 */
async function cleanStructuralNodes(node, tagNameMap, target) {
    ...
    const cleanedChildrenPromises = node.children.map(child => cleanStructuralNodes(child, tagNameMap, target));
    const resolvedChildren = await Promise.all(cleanedChildrenPromises);
    const cleanedChildren = resolvedChildren.filter((child) => {
        if (child === null) {
            return false;
        }
 ...
 */
export async function buildHierarchicalTree(nodes, target, 
// Add parameter to receive scrollable IDs
scrollableBackendIds) {
    ...
    const rootNodes = nodes
        ...
        .filter(Boolean);
    const cleanedTreePromises = rootNodes.map(node => cleanStructuralNodes(node, tagNameMap, target));
    ...
    const iframesWithContent = [];
    if (accessibilityAgent && iframeList.length > 0) {
        const iframeContentPromises = iframeList.map(async (iframe) => {
            try {
                const domAgent = target.domAgent();
                ...
                const iframeAccessibilityNodes = iframeResponse.nodes.map((iframeNode) => {
                    ...
                    return {
                        ...
                        childIds: iframeNode.childIds?.map((childId) => `${iframePrefix}${childId}`),
                    };
                });
        ...
        iframesWithContent.push(...iframeList.map(iframe => iframe));
    }
 ...
 */
export async function getAccessibilityTree(target) {
    try {
        ...
        const hierarchicalTree = await buildHierarchicalTree(accessibilityNodes, target, scrollableBackendIds);
...
export async function getXPathByResolvedObjectId(target, resolvedObjectId) {
 ...
 */
export async function getXPathByBackendNodeId(target, backendNodeId) {
    try {
 ...
 */
async function initializeScrollableElementsFunction(target) {
    const runtimeAgent = target.runtimeAgent();
 ...
 */
export async function findScrollableElementIds(target) {
...
export async function performAction(target, method, args, xpath, iframeNodeId) {
    const runtimeAgent = target.runtimeAgent();
    const domAgent = target.domAgent();
    let objectId;
        ...
        if (method === 'click') {
          ...
          `,
                ...
                await inputAgent.invoke_dispatchMouseEvent({
                    type: "mousePressed" /* Protocol.Input.DispatchMouseEventRequestType.MousePressed */,
                    x,
                    y,
                    button: "left" /* Protocol.Input.MouseButton.Left */,
                    clickCount: 1
                });
                await inputAgent.invoke_dispatchMouseEvent({
                    type: "mouseReleased" /* Protocol.Input.DispatchMouseEventRequestType.MouseReleased */,
                    x,
                    y,
                    button: "left" /* Protocol.Input.MouseButton.Left */,
                    clickCount: 1
            ...
            // Perform hover
            await inputAgent.invoke_dispatchMouseEvent({
                type: "mouseMoved" /* Protocol.Input.DispatchMouseEventRequestType.MouseMoved */,
                x,
                y
            });
        }
 ...
 */
export async function getVisibleAccessibilityTree(target) {
    const startTime = Date.now();
    ...
    try {
        ...
        // --- DEBUG LOG: Full Tree ---
        try {
            const fullAccessibilityNodes = allCdpNodes.map((node) => {
        ...
        const cleanedRootPromises = rootNodes.map(node => cleanStructuralNodes(node, tagNameMap, target));
        const finalTree = (await Promise.all(cleanedRootPromises)).filter(Boolean);
        // 9. Format Output
        ...
        const allAccessibilityNodes = allCdpNodes.map((node) => {
            const roleValue = node.role && typeof node.role === 'object' && 'value' in node.role
                ? node.role.value
                : '';
            const nameValue = node.name && typeof node.name === 'object' && 'value' in node.name
        ...
        const iframesWithContent = await Promise.all(iframeNodes.map(async (node) => {
            ...
            try {
                ...
                const iframeAccessibilityNodes = iframeResponse.nodes.map((iframeNode) => {
                    ...
                    const nameValue = iframeNode.name && typeof iframeNode.name === 'object' && 'value' in iframeNode.name
                        ...
                        : undefined;
                    const descriptionValue = iframeNode.description &&
                        typeof iframeNode.description === 'object' &&
                        'value' in iframeNode.description
                        ? iframeNode.description.value
                        : undefined;
                    const valueValue = iframeNode.value && typeof iframeNode.value === 'object' && 'value' in iframeNode.value
        ...
        if (iframesWithContent.length > 0) {
            simplified += '\n\n--- IFRAME CONTENT ---\n';
            iframesWithContent.forEach((iframe, index) => {
                const iframeWithContent = iframe;
 ...
 */
async function findElementsInViewport(target) {
