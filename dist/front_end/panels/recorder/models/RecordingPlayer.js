...
var _RecordingPlayer_instances, _RecordingPlayer_stopResolver, _RecordingPlayer_abortResolver, _RecordingPlayer_runner, _RecordingPlayer_resolveAndRefreshStopPromise;
...
export class RecordingPlayer extends Common.ObjectWrapper.ObjectWrapper {
    constructor(userFlow, { speed, breakpointIndexes = new Set(), }) {
        ...
        this.aborted = false;
        _RecordingPlayer_stopResolver.set(this, Promise.withResolvers());
        _RecordingPlayer_abortResolver.set(this, Promise.withResolvers());
    ...
    async stop() {
        await Promise.race([__classPrivateFieldGet(this, _RecordingPlayer_stopResolver, "f").promise, __classPrivateFieldGet(this, _RecordingPlayer_abortResolver, "f").promise]);
    }
    get abortPromise() {
        return __classPrivateFieldGet(this, _RecordingPlayer_abortResolver, "f").promise;
    }
    abort() {
        this.aborted = true;
        __classPrivateFieldGet(this, _RecordingPlayer_abortResolver, "f").resolve();
        __classPrivateFieldGet(this, _RecordingPlayer_runner, "f")?.abort();
    }
    disposeForTesting() {
        __classPrivateFieldGet(this, _RecordingPlayer_stopResolver, "f").resolve();
        __classPrivateFieldGet(this, _RecordingPlayer_abortResolver, "f").resolve();
    }
    continue() {
        this.steppingOver = false;
    ...
    async play() {
        ...
        __classPrivateFieldSet(this, _RecordingPlayer_runner, await PuppeteerReplay.createRunner(this.userFlow, extension), "f");
        let error;
        try {
...
_RecordingPlayer_stopResolver = new WeakMap(), _RecordingPlayer_abortResolver = new WeakMap(), _RecordingPlayer_runner = new WeakMap(), _RecordingPlayer_instances = new WeakSet(), _RecordingPlayer_resolveAndRefreshStopPromise = function _RecordingPlayer_resolveAndRefreshStopPromise() {
    __classPrivateFieldGet(this, _RecordingPlayer_stopResolver, "f").resolve();
    __classPrivateFieldSet(this, _RecordingPlayer_stopResolver, Promise.withResolvers(), "f");
};
export var Events;
(function (Events) {
    Events["ABORT"] = "Abort";
