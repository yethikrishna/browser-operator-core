...
var _TimelinePaintProfilerView_instances, _TimelinePaintProfilerView_parsedTrace, _TimelinePaintProfilerView_rasterEventHasTile, _TimelinePaintProfilerView_rasterTilePromise;
...
export class TimelinePaintProfilerView extends UI.SplitWidget.SplitWidget {
    constructor(parsedTrace) {
        super(false, false);
        _TimelinePaintProfilerView_instances.add(this);
        _TimelinePaintProfilerView_parsedTrace.set(this, void 0);
        this.element.classList.add('timeline-paint-profiler-view');
        this.setSidebarSize(60);
        this.setResizable(false);
        __classPrivateFieldSet(this, _TimelinePaintProfilerView_parsedTrace, parsedTrace, "f");
    ...
    setEvent(paintProfilerModel, event) {
        ...
        if (Trace.Types.Events.isPaint(event)) {
            const snapshot = __classPrivateFieldGet(this, _TimelinePaintProfilerView_parsedTrace, "f").LayerTree.paintsToSnapshots.get(event);
            return Boolean(snapshot);
        }
        if (Trace.Types.Events.isRasterTask(event)) {
            return __classPrivateFieldGet(this, _TimelinePaintProfilerView_instances, "m", _TimelinePaintProfilerView_rasterEventHasTile).call(this, event);
        }
        return false;
    }
    updateWhenVisible() {
    ...
    update() {
        ...
        if (this.pendingSnapshot) {
            ...
            // it.
            const snapshotEvent = __classPrivateFieldGet(this, _TimelinePaintProfilerView_parsedTrace, "f").LayerTree.paintsToSnapshots.get(this.event);
            if (snapshotEvent) {
            ...
            snapshotPromise = __classPrivateFieldGet(this, _TimelinePaintProfilerView_instances, "m", _TimelinePaintProfilerView_rasterTilePromise).call(this, this.event);
        }
        else {
...
_TimelinePaintProfilerView_parsedTrace = new WeakMap(), _TimelinePaintProfilerView_instances = new WeakSet(), _TimelinePaintProfilerView_rasterEventHasTile = function _TimelinePaintProfilerView_rasterEventHasTile(event) {
    const data = event.args.tileData;
    if (!data) {
        return false;
    }
    const frame = __classPrivateFieldGet(this, _TimelinePaintProfilerView_parsedTrace, "f").Frames.framesById[data.sourceFrameNumber];
    if (!frame?.layerTree) {
        return false;
    }
    return true;
}, _TimelinePaintProfilerView_rasterTilePromise = async function _TimelinePaintProfilerView_rasterTilePromise(rasterEvent) {
