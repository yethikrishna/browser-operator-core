...
var _TimelineTreeView_instances, _TimelineTreeView_selectedEvents, _TimelineTreeView_parsedTrace, _TimelineTreeView_entityMapper, _TimelineTreeView_lastHighlightedEvent, _TimelineTreeView_eventNameForSorting, _TimelineTreeView_filterChanged, _TimelineTreeView_onDataGridSelectionChange, _TimelineTreeView_onDataGridDeselection, _GridNode_instances, _GridNode_bottomUpButtonClicked, _AggregatedTimelineTreeView_instances, _AggregatedTimelineTreeView_getThirdPartyEventsForNode;
...
export class TimelineTreeView extends Common.ObjectWrapper.eventMixin(UI.Widget.VBox) {
    constructor() {
        super();
        _TimelineTreeView_instances.add(this);
        _TimelineTreeView_selectedEvents.set(this, void 0);
        _TimelineTreeView_parsedTrace.set(this, null);
        _TimelineTreeView_entityMapper.set(this, null);
        _TimelineTreeView_lastHighlightedEvent.set(this, null);
        this.eventToTreeNode = new WeakMap();
        /**
         * Determines if the first child in the data grid will be selected
         * by default when refreshTree() gets called.
         */
        this.autoSelectFirstChildOnRefresh = true;
        __classPrivateFieldSet(this, _TimelineTreeView_selectedEvents, null, "f");
    ...
    setModelWithEvents(selectedEvents, parsedTrace = null, entityMappings = null) {
        __classPrivateFieldSet(this, _TimelineTreeView_parsedTrace, parsedTrace, "f");
        __classPrivateFieldSet(this, _TimelineTreeView_selectedEvents, selectedEvents, "f");
        __classPrivateFieldSet(this, _TimelineTreeView_entityMapper, entityMappings, "f");
        this.refreshTree();
    }
    entityMapper() {
        return __classPrivateFieldGet(this, _TimelineTreeView_entityMapper, "f");
    }
    parsedTrace() {
        return __classPrivateFieldGet(this, _TimelineTreeView_parsedTrace, "f");
    }
    init() {
    ...
    highlightEventInTree(event) {
        ...
        if (!event || (dataGridElem && dataGridElem !== __classPrivateFieldGet(this, _TimelineTreeView_lastHighlightedEvent, "f"))) {
            __classPrivateFieldGet(this, _TimelineTreeView_lastHighlightedEvent, "f")?.style.setProperty('background-color', '');
        }
        if (event) {
            const rowElem = dataGridElem;
            if (rowElem) {
                __classPrivateFieldSet(this, _TimelineTreeView_lastHighlightedEvent, rowElem, "f");
                __classPrivateFieldGet(this, _TimelineTreeView_lastHighlightedEvent, "f").style.backgroundColor = 'var(--sys-color-yellow-container)';
            }
        }
    }
    filters() {
    ...
    populateToolbar(toolbar) {
        ...
        this.caseSensitiveButton.addEventListener("Click" /* UI.Toolbar.ToolbarButton.Events.CLICK */, () => {
            __classPrivateFieldGet(this, _TimelineTreeView_instances, "m", _TimelineTreeView_filterChanged).call(this);
        }, this);
        ...
        textFilterUI.addEventListener("TextChanged" /* UI.Toolbar.ToolbarInput.Event.TEXT_CHANGED */, __classPrivateFieldGet(this, _TimelineTreeView_instances, "m", _TimelineTreeView_filterChanged), this);
        toolbar.appendToolbarItem(textFilterUI);
    }
    selectedEvents() {
        // TODO: can we make this type readonly?
        return __classPrivateFieldGet(this, _TimelineTreeView_selectedEvents, "f") || [];
    }
    ...
    refreshTree() {
        ...
        if (!__classPrivateFieldGet(this, _TimelineTreeView_parsedTrace, "f")) {
    ...
    getSortingFunction(columnId) {
        const compareNameSortFn = (a, b) => {
            ...
            const nameA = __classPrivateFieldGet(this, _TimelineTreeView_instances, "m", _TimelineTreeView_eventNameForSorting).call(this, eventA);
            const nameB = __classPrivateFieldGet(this, _TimelineTreeView_instances, "m", _TimelineTreeView_eventNameForSorting).call(this, eventB);
    ...
    wasShown() {
        this.dataGrid.addEventListener("SelectedNode" /* DataGrid.DataGrid.Events.SELECTED_NODE */, __classPrivateFieldGet(this, _TimelineTreeView_instances, "m", _TimelineTreeView_onDataGridSelectionChange), this);
        this.dataGrid.addEventListener("DeselectedNode" /* DataGrid.DataGrid.Events.DESELECTED_NODE */, __classPrivateFieldGet(this, _TimelineTreeView_instances, "m", _TimelineTreeView_onDataGridDeselection), this);
    }
    childWasDetached(_widget) {
        this.dataGrid.removeEventListener("SelectedNode" /* DataGrid.DataGrid.Events.SELECTED_NODE */, __classPrivateFieldGet(this, _TimelineTreeView_instances, "m", _TimelineTreeView_onDataGridSelectionChange));
        this.dataGrid.removeEventListener("DeselectedNode" /* DataGrid.DataGrid.Events.DESELECTED_NODE */, __classPrivateFieldGet(this, _TimelineTreeView_instances, "m", _TimelineTreeView_onDataGridDeselection));
    }
    onGridNodeOpened() {
    ...
    performSearch(searchConfig, _shouldJump, _jumpBackwards) {
        ...
        this.searchResults = this.root.searchTree(event => TimelineUIUtils.testContentMatching(event, searchRegex.regex, __classPrivateFieldGet(this, _TimelineTreeView_parsedTrace, "f") || undefined));
...
_TimelineTreeView_selectedEvents = new WeakMap(), _TimelineTreeView_parsedTrace = new WeakMap(), _TimelineTreeView_entityMapper = new WeakMap(), _TimelineTreeView_lastHighlightedEvent = new WeakMap(), _TimelineTreeView_instances = new WeakSet(), _TimelineTreeView_eventNameForSorting = function _TimelineTreeView_eventNameForSorting(event) {
    const name = TimelineUIUtils.eventTitle(event) || event.name;
    if (!__classPrivateFieldGet(this, _TimelineTreeView_parsedTrace, "f")) {
        return name;
    }
    return name + ':@' + Trace.Handlers.Helpers.getNonResolvedURL(event, __classPrivateFieldGet(this, _TimelineTreeView_parsedTrace, "f"));
}, _TimelineTreeView_filterChanged = function _TimelineTreeView_filterChanged() {
    ...
    this.textFilterInternal.setRegExp(searchQuery ? Platform.StringUtilities.createSearchRegex(searchQuery, caseSensitive, isRegex, matchWholeWord) :
        null);
    this.refreshTree();
}, _TimelineTreeView_onDataGridSelectionChange = function _TimelineTreeView_onDataGridSelectionChange(event) {
    this.onClick(event.data.profileNode);
    this.onHover(event.data.profileNode);
}, _TimelineTreeView_onDataGridDeselection = function _TimelineTreeView_onDataGridDeselection() {
    this.onClick(null);
    this.onHover(null);
};
...
export class GridNode extends DataGrid.SortableDataGrid.SortableDataGridNode {
    ...
    generateBottomUpButton(textDiv) {
        ...
        button.addEventListener('click', () => __classPrivateFieldGet(this, _GridNode_instances, "m", _GridNode_bottomUpButtonClicked).call(this));
        UI.Tooltip.Tooltip.install(button, i18nString(UIStrings.bottomUp));
        // Append the button to the last column
        textDiv.appendChild(button);
    }
}
_GridNode_instances = new WeakSet(), _GridNode_bottomUpButtonClicked = function _GridNode_bottomUpButtonClicked() {
...
export class AggregatedTimelineTreeView extends TimelineTreeView {
    constructor() {
        super();
        _AggregatedTimelineTreeView_instances.add(this);
    ...
    onHover(node) {
        if (node !== null && this.groupBySetting.get() === AggregatedTimelineTreeView.GroupBy.ThirdParties) {
            const events = __classPrivateFieldGet(this, _AggregatedTimelineTreeView_instances, "m", _AggregatedTimelineTreeView_getThirdPartyEventsForNode).call(this, node);
...
_AggregatedTimelineTreeView_instances = new WeakSet(), _AggregatedTimelineTreeView_getThirdPartyEventsForNode = function _AggregatedTimelineTreeView_getThirdPartyEventsForNode(node) {
    if (!node.event) {
        return;
    }
...
AggregatedTimelineTreeView.extensionInternalPrefix = 'extensions::';
AggregatedTimelineTreeView.v8NativePrefix = 'native ';
