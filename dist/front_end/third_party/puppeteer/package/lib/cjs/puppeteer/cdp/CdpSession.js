...
class CdpCDPSession extends CDPSession_js_1.CDPSession {
    ...
    constructor(connection, targetType, sessionId, parentSessionId, rawErrors) {
        ...
        _CdpCDPSession_callbacks.set(this, new CallbackRegistry_js_1.CallbackRegistry());
     ...
     */
    target() {
        (0, assert_js_1.assert)(__classPrivateFieldGet(this, _CdpCDPSession_target, "f"), 'Target must exist');
    ...
    send(method, params, options) {
        if (this.detached) {
            return Promise.reject(new Errors_js_1.TargetCloseError(`Protocol error (${method}): Session closed. Most likely the ${__classPrivateFieldGet(this, _CdpCDPSession_targetType, "f")} has been closed.`));
        }
        ...
        if (object.id) {
            if (object.error) {
                if (__classPrivateFieldGet(this, _CdpCDPSession_rawErrors, "f")) {
                    ...
                    __classPrivateFieldGet(this, _CdpCDPSession_callbacks, "f").reject(object.id, (0, ErrorLike_js_1.createProtocolErrorMessage)(object), object.error.message);
                }
            }
            else {
