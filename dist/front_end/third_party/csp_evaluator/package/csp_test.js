...
describe('Test Csp', () => {
    ...
    it('GetEffectiveDirectives', () => {
        const testCsp = 'default-src https:; script-src foo.bar';
        const parsed = (new CspParser(testCsp)).csp;
        const directives = parsed.getEffectiveDirectives([Directive.SCRIPT_SRC, Directive.STYLE_SRC]);
    ...
    it('IsDirective', () => {
        const directives = Object.keys(Directive).map((name) => Directive[name]);
        expect(directives.every(isDirective)).toBeTrue();
        expect(isDirective('invalid-src')).toBeFalse();
    });
    it('IsKeyword', () => {
        const keywords = Object.keys(Keyword).map((name) => (Keyword[name]));
