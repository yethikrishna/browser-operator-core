...
export class TraceLoader {
    ...
    static async traceEngine(context, name, config = Trace.Types.Configuration.defaults()) {
        ...
        if (fromCache) {
            ...
            }, 4000, 'Initializing state for cached trace');
        ...
        }, 15000, `Loading fixtureContents for ${name}`);
        ...
        }, 15000, `Executing traceEngine for ${name}`);
        ...
        }, 5000, `Creating modification manager for ${name}`);
        return {
