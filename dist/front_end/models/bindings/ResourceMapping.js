...
var _ResourceMapping_modelToInfo, _ModelInfo_bindings, _ModelInfo_cssModel, _ModelInfo_eventListeners, _Binding_project, _Binding_uiSourceCode, _Binding_edits;
...
export class ResourceMapping {
    constructor(targetManager, workspace) {
        _ResourceMapping_modelToInfo.set(this, new Map());
    ...
    modelAdded(resourceTreeModel) {
        ...
        __classPrivateFieldGet(this, _ResourceMapping_modelToInfo, "f").set(resourceTreeModel, info);
    }
    modelRemoved(resourceTreeModel) {
        const info = __classPrivateFieldGet(this, _ResourceMapping_modelToInfo, "f").get(resourceTreeModel);
        if (info) {
            info.dispose();
            __classPrivateFieldGet(this, _ResourceMapping_modelToInfo, "f").delete(resourceTreeModel);
        }
    }
    infoForTarget(target) {
        const resourceTreeModel = target.model(SDK.ResourceTreeModel.ResourceTreeModel);
        return resourceTreeModel ? __classPrivateFieldGet(this, _ResourceMapping_modelToInfo, "f").get(resourceTreeModel) || null : null;
    }
    ...
    resetForTest(target) {
        const resourceTreeModel = target.model(SDK.ResourceTreeModel.ResourceTreeModel);
        const info = resourceTreeModel ? __classPrivateFieldGet(this, _ResourceMapping_modelToInfo, "f").get(resourceTreeModel) : null;
        if (info) {
            info.resetForTest();
        }
    }
}
_ResourceMapping_modelToInfo = new WeakMap();
class ModelInfo {
    constructor(workspace, resourceTreeModel) {
        _ModelInfo_bindings.set(this, new Map());
        _ModelInfo_cssModel.set(this, void 0);
        ...
        __classPrivateFieldSet(this, _ModelInfo_cssModel, cssModel, "f");
        ...
        __classPrivateFieldSet(this, _ModelInfo_eventListeners, [
            ...
            __classPrivateFieldGet(this, _ModelInfo_cssModel, "f").addEventListener(SDK.CSSModel.Events.StyleSheetChanged, event => {
                void this.styleSheetChanged(event);
            }, this),
        ], "f");
    }
    async styleSheetChanged(event) {
        const header = __classPrivateFieldGet(this, _ModelInfo_cssModel, "f").styleSheetHeaderForId(event.data.styleSheetId);
        if (!header || !header.isInline || (header.isInline && header.isMutable)) {
            return;
        }
        const binding = __classPrivateFieldGet(this, _ModelInfo_bindings, "f").get(header.resourceURL());
        if (!binding) {
            return;
        }
    ...
    addResource(resource) {
        ...
        let binding = __classPrivateFieldGet(this, _ModelInfo_bindings, "f").get(resource.url);
        if (!binding) {
            binding = new Binding(this.project, resource);
            __classPrivateFieldGet(this, _ModelInfo_bindings, "f").set(resource.url, binding);
        }
        else {
    ...
    removeFrameResources(frame) {
        for (const resource of frame.resources()) {
            ...
            const binding = __classPrivateFieldGet(this, _ModelInfo_bindings, "f").get(resource.url);
            if (!binding) {
                continue;
            }
            if (binding.resources.size === 1) {
                binding.dispose();
                __classPrivateFieldGet(this, _ModelInfo_bindings, "f").delete(resource.url);
            }
            else {
    ...
    resetForTest() {
        for (const binding of __classPrivateFieldGet(this, _ModelInfo_bindings, "f").values()) {
            binding.dispose();
        }
        __classPrivateFieldGet(this, _ModelInfo_bindings, "f").clear();
    }
    dispose() {
...
_ModelInfo_bindings = new WeakMap(), _ModelInfo_cssModel = new WeakMap(), _ModelInfo_eventListeners = new WeakMap();
class Binding {
    constructor(project, resource) {
        _Binding_project.set(this, void 0);
        _Binding_uiSourceCode.set(this, void 0);
        _Binding_edits.set(this, []);
        this.resources = new Set([resource]);
        __classPrivateFieldSet(this, _Binding_project, project, "f");
        __classPrivateFieldSet(this, _Binding_uiSourceCode, __classPrivateFieldGet(this, _Binding_project, "f").createUISourceCode(resource.url, resource.contentType()), "f");
        boundUISourceCodes.add(__classPrivateFieldGet(this, _Binding_uiSourceCode, "f"));
        if (resource.frameId) {
            NetworkProject.setInitialFrameAttribution(__classPrivateFieldGet(this, _Binding_uiSourceCode, "f"), resource.frameId);
        }
        __classPrivateFieldGet(this, _Binding_project, "f").addUISourceCodeWithProvider(__classPrivateFieldGet(this, _Binding_uiSourceCode, "f"), this, resourceMetadata(resource), resource.mimeType);
        void Promise.all([
    ...
    inlineStyles() {
        const target = NetworkProject.targetForUISourceCode(__classPrivateFieldGet(this, _Binding_uiSourceCode, "f"));
        const stylesheets = [];
        if (!target) {
            return stylesheets;
        }
        const cssModel = target.model(SDK.CSSModel.CSSModel);
        if (cssModel) {
            for (const headerId of cssModel.getStyleSheetIdsForURL(__classPrivateFieldGet(this, _Binding_uiSourceCode, "f").url())) {
    ...
    inlineScripts() {
        ...
        return debuggerModel.scripts().filter(script => script.embedderName() === __classPrivateFieldGet(this, _Binding_uiSourceCode, "f").url());
    }
    async styleSheetChanged(stylesheet, edit) {
        __classPrivateFieldGet(this, _Binding_edits, "f").push({ stylesheet, edit });
        if (__classPrivateFieldGet(this, _Binding_edits, "f").length > 1) {
            return;
        } // There is already a styleSheetChanged loop running
        const content = await __classPrivateFieldGet(this, _Binding_uiSourceCode, "f").requestContentData();
        if (!TextUtils.ContentData.ContentData.isError(content)) {
            await this.innerStyleSheetChanged(content.text);
        }
        __classPrivateFieldSet(this, _Binding_edits, [], "f");
    }
    async innerStyleSheetChanged(content) {
        ...
        for (const data of __classPrivateFieldGet(this, _Binding_edits, "f")) {
        ...
        __classPrivateFieldGet(this, _Binding_uiSourceCode, "f").addRevision(text.value());
    }
    addResource(resource) {
        this.resources.add(resource);
        if (resource.frameId) {
            NetworkProject.addFrameAttribution(__classPrivateFieldGet(this, _Binding_uiSourceCode, "f"), resource.frameId);
        }
    }
    removeResource(resource) {
        this.resources.delete(resource);
        if (resource.frameId) {
            NetworkProject.removeFrameAttribution(__classPrivateFieldGet(this, _Binding_uiSourceCode, "f"), resource.frameId);
        }
    }
    dispose() {
        __classPrivateFieldGet(this, _Binding_project, "f").removeUISourceCode(__classPrivateFieldGet(this, _Binding_uiSourceCode, "f").url());
        void Promise.all([
            ...this.inlineScripts().map(script => DebuggerWorkspaceBinding.instance().updateLocations(script)),
            ...this.inlineStyles().map(style => CSSWorkspaceBinding.instance().updateLocations(style)),
        ]);
...
_Binding_project = new WeakMap(), _Binding_uiSourceCode = new WeakMap(), _Binding_edits = new WeakMap();
