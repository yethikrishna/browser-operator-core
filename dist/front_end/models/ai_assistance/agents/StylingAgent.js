...
var _NodeContext_node, _StylingAgent_execJs, _StylingAgent_changes, _StylingAgent_createExtensionScope;
...
const OBSERVATION_TIMEOUT = 5000;
export class NodeContext extends ConversationContext {
    constructor(node) {
        super();
        _NodeContext_node.set(this, void 0);
        __classPrivateFieldSet(this, _NodeContext_node, node, "f");
    }
    getOrigin() {
        const ownerDocument = __classPrivateFieldGet(this, _NodeContext_node, "f").ownerDocument;
        if (!ownerDocument) {
    ...
    getItem() {
        return __classPrivateFieldGet(this, _NodeContext_node, "f");
    }
    getIcon() {
    }
    getTitle(opts) {
        const hiddenClassList = __classPrivateFieldGet(this, _NodeContext_node, "f").classNames().filter(className => className.startsWith(AI_ASSISTANCE_CSS_CLASS_NAME));
        const { DOMNodeLink } = ElementsPanel.DOMLinkifier;
        const { widgetConfig } = UI.Widget;
        return html `<devtools-widget .widgetConfig=${widgetConfig(DOMNodeLink, { node: __classPrivateFieldGet(this, _NodeContext_node, "f"), options: { hiddenClassList, disabled: opts.disabled } })}></devtools-widget>`;
    }
    async getSuggestions() {
        const layoutProps = await __classPrivateFieldGet(this, _NodeContext_node, "f").domModel().cssModel().getLayoutPropertiesFromComputedStyle(__classPrivateFieldGet(this, _NodeContext_node, "f").id);
        if (!layoutProps) {
            return;
        }
...
export class StylingAgent extends AiAgent {
    ...
    constructor(opts) {
        ...
        this.functionCallEmulationEnabled = true;
        this.preamble = preamble;
        this.clientFeature = Host.AidaClient.ClientFeature.CHROME_STYLING_AGENT;
        _StylingAgent_execJs.set(this, void 0);
        _StylingAgent_changes.set(this, void 0);
        _StylingAgent_createExtensionScope.set(this, void 0);
        __classPrivateFieldSet(this, _StylingAgent_changes, opts.changeManager || new ChangeManager(), "f");
        __classPrivateFieldSet(this, _StylingAgent_execJs, opts.execJs ?? executeJsCode, "f");
        __classPrivateFieldSet(this, _StylingAgent_createExtensionScope, opts.createExtensionScope ?? ((changes) => {
...
`,
    ...
    onPrimaryPageChanged() {
        void __classPrivateFieldGet(this, _StylingAgent_changes, "f").clear();
    }
  ...
  try {
    ...
    return error;
        ...
        try {
            const result = await Promise.race([
                __classPrivateFieldGet(this, _StylingAgent_execJs, "f").call(this, functionDeclaration, { throwOnSideEffect, contextNode: this.context?.getItem() || null }),
    ...
    async executeAction(action, options) {
        ...
        const scope = __classPrivateFieldGet(this, _StylingAgent_createExtensionScope, "f").call(this, __classPrivateFieldGet(this, _StylingAgent_changes, "f"));
        await scope.install();
        try {
...
_StylingAgent_execJs = new WeakMap(), _StylingAgent_changes = new WeakMap(), _StylingAgent_createExtensionScope = new WeakMap();
/* clang-format off */
...
export class StylingAgentWithFunctionCalling extends StylingAgent {
    constructor() {
        ...
        this.preamble = preambleFunctionCalling;
    }
    formatParsedAnswer({ answer }) {
        return answer;
    }
