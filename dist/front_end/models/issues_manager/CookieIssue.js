...
(function (CookieIssueSubCategory) {
    ...
    CookieIssueSubCategory["THIRD_PARTY_PHASEOUT_COOKIE"] = "ThirdPartyPhaseoutCookie";
...
export class CookieIssue extends Issue {
    constructor(code, issueDetails, issuesModel, issueId) {
        ...
        _CookieIssue_issueDetails.set(this, void 0);
        __classPrivateFieldSet(this, _CookieIssue_issueDetails, issueDetails, "f");
    }
    cookieId() {
        if (__classPrivateFieldGet(this, _CookieIssue_issueDetails, "f").cookie) {
            const { domain, path, name } = __classPrivateFieldGet(this, _CookieIssue_issueDetails, "f").cookie;
            const cookieId = `${domain};${path};${name}`;
            return cookieId;
        }
        return __classPrivateFieldGet(this, _CookieIssue_issueDetails, "f").rawCookieLine ?? 'no-cookie-info';
    }
    primaryKey() {
        const requestId = __classPrivateFieldGet(this, _CookieIssue_issueDetails, "f").request ? __classPrivateFieldGet(this, _CookieIssue_issueDetails, "f").request.requestId : 'no-request';
    ...
    cookies() {
        if (__classPrivateFieldGet(this, _CookieIssue_issueDetails, "f").cookie) {
            return [__classPrivateFieldGet(this, _CookieIssue_issueDetails, "f").cookie];
        }
        return [];
    }
    rawCookieLines() {
        if (__classPrivateFieldGet(this, _CookieIssue_issueDetails, "f").rawCookieLine) {
            return [__classPrivateFieldGet(this, _CookieIssue_issueDetails, "f").rawCookieLine];
        }
        return [];
    }
    requests() {
        if (__classPrivateFieldGet(this, _CookieIssue_issueDetails, "f").request) {
            return [__classPrivateFieldGet(this, _CookieIssue_issueDetails, "f").request];
        }
        return [];
    }
    getCategory() {
    ...
    isCausedByThirdParty() {
        ...
        return isCausedByThirdParty(outermostFrame, __classPrivateFieldGet(this, _CookieIssue_issueDetails, "f").cookieUrl, __classPrivateFieldGet(this, _CookieIssue_issueDetails, "f").siteForCookies);
    }
    getKind() {
        if (__classPrivateFieldGet(this, _CookieIssue_issueDetails, "f").cookieExclusionReasons?.length > 0) {
            return "PageError" /* IssueKind.PAGE_ERROR */;
        }
        return "BreakingChange" /* IssueKind.BREAKING_CHANGE */;
    }
    makeCookieReportEntry() {
        const status = CookieIssue.getCookieStatus(__classPrivateFieldGet(this, _CookieIssue_issueDetails, "f"));
        if (__classPrivateFieldGet(this, _CookieIssue_issueDetails, "f").cookie && __classPrivateFieldGet(this, _CookieIssue_issueDetails, "f").cookieUrl && status !== undefined) {
            const entity = ThirdPartyWeb.ThirdPartyWeb.getEntity(__classPrivateFieldGet(this, _CookieIssue_issueDetails, "f").cookieUrl);
            return {
                name: __classPrivateFieldGet(this, _CookieIssue_issueDetails, "f").cookie.name,
                domain: __classPrivateFieldGet(this, _CookieIssue_issueDetails, "f").cookie.domain,
                type: entity?.category,
                platform: entity?.name,
                status,
                insight: __classPrivateFieldGet(this, _CookieIssue_issueDetails, "f").insight,
            };
        }
        return;
    }
    ...
    maybeCreateConsoleMessage() {
        ...
        if (issuesModel && this.code().includes("ExcludeThirdPartyPhaseout" /* Protocol.Audits.CookieExclusionReason.ExcludeThirdPartyPhaseout */)) {
            return new SDK.ConsoleModel.ConsoleMessage(issuesModel.target().model(SDK.RuntimeModel.RuntimeModel), Common.Console.FrontendMessageSource.ISSUE_PANEL, "warning" /* Protocol.Log.LogEntryLevel.Warning */, UIStrings.consoleTpcdErrorMessage, {
                url: __classPrivateFieldGet(this, _CookieIssue_issueDetails, "f").request?.url,
                affectedResources: { requestId: __classPrivateFieldGet(this, _CookieIssue_issueDetails, "f").request?.requestId, issueId: this.issueId },
...
_CookieIssue_issueDetails = new WeakMap();
/**
