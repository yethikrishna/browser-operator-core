...
var _DiffRenderer_instances, _DiffRenderer_render, _DiffRenderer_renderRow, _DiffRenderer_renderRowContent, _DiffView_instances, _DiffView_shadow, _DiffView_render;
...
class DiffRenderer {
    ...
    static async render(diff, mimeType, parent) {
        ...
        Lit.render(__classPrivateFieldGet(renderer, _DiffRenderer_instances, "m", _DiffRenderer_render).call(renderer, rows), parent, { host: this });
    }
}
_DiffRenderer_instances = new WeakSet(), _DiffRenderer_render = function _DiffRenderer_render(rows) {
    ...
    return html `
      ...
      <div class="diff-line-content diff-line-${row.type}" data-line-number=${curNumber} jslog=${VisualLogging.link('changes.reveal-source').track({ click: true })}>${screenReaderText}${__classPrivateFieldGet(this, _DiffRenderer_instances, "m", _DiffRenderer_renderRowContent).call(this, row)}</div>`;
...
export class DiffView extends HTMLElement {
    constructor(data) {
        super();
        _DiffView_instances.add(this);
        _DiffView_shadow.set(this, this.attachShadow({ mode: 'open' }));
        this.loaded = __classPrivateFieldGet(this, _DiffView_instances, "m", _DiffView_render).call(this, data);
    }
    set data(data) {
        this.loaded = __classPrivateFieldGet(this, _DiffView_instances, "m", _DiffView_render).call(this, data);
    }
}
_DiffView_shadow = new WeakMap(), _DiffView_instances = new WeakSet(), _DiffView_render = async function _DiffView_render(data) {
    if (!data || data.diff.length === 0) {
        renderNoDiffState(__classPrivateFieldGet(this, _DiffView_shadow, "f"));
        return;
    }
    await DiffRenderer.render(data.diff, data.mimeType, __classPrivateFieldGet(this, _DiffView_shadow, "f"));
};
