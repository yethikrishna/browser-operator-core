...
export const waitUntilPausedAndDumpStackAndResume = function (callback, options) {
    ...
    TestRunner.addSniffer(Sources.SourcesPanel.SourcesPanel.prototype, 'updateDebuggerButtonsAndStatusForTest', setStatus);
    let caption;
    let callFrames;
    let asyncStackTrace;
    function setStatus() {
...
export const captureStackTraceIntoString = async function (callFrames, asyncStackTrace, options) {
    ...
    await printCallFrames(callFrames, SDK.DebuggerModel.CallFrame.prototype.location, SDK.DebuggerModel.CallFrame.prototype.returnValue);
...
export const waitForScriptSource = function (scriptName, callback, contentType) {
    ...
    TestRunner.addSniffer(Sources.SourcesView.SourcesView.prototype, 'addUISourceCode', waitForScriptSource.bind(undefined, scriptName, callback, contentType));
};
...
export const removeBreakpoint = function (sourceFrame, lineNumber) {
    ...
    const breakpointLocation = breakpointLocations.find(breakpointLocation => breakpointLocation.uiLocation.uiSourceCode === sourceFrame.uiSourceCode() &&
...
export const expandProperty = function (parentTreeElement, path, pathIndex, callback) {
    ...
    TestRunner.deprecatedRunAfterPendingDispatches(expandProperty.bind(undefined, propertyTreeElement, path, pathIndex, callback));
};
...
export const checkUILocation = function (uiSourceCode, lineNumber, columnNumber, location) {
    TestRunner.assertEquals(uiSourceCode, location.uiSourceCode, 'Incorrect uiSourceCode, expected \'' + ((uiSourceCode ? uiSourceCode.url() : null)) + '\',' +
        ' but got \'' + ((location.uiSourceCode ? location.uiSourceCode.url() : null)) + '\'');
    TestRunner.assertEquals(lineNumber, location.lineNumber, 'Incorrect lineNumber, expected \'' + lineNumber + '\', but got \'' + location.lineNumber + '\'');
    TestRunner.assertEquals(columnNumber, location.columnNumber, 'Incorrect columnNumber, expected \'' + columnNumber + '\', but got \'' + location.columnNumber + '\'');
};
...
`);
//# sourceMappingURL=DebuggerTestRunner.js.map