...
export const overrideNetworkModificationTime = function (urlToTime) {
    if (!timeOverrides) {
        ...
        originalRequestMetadata = TestRunner.override(Bindings.ContentProviderBasedProject.ContentProviderBasedProject.prototype, 'requestMetadata', overrideTime, true);
    }
...
export const AutomappingTest = function (workspace) {
    ...
    this.networkProject = new Bindings.ContentProviderBasedProject.ContentProviderBasedProject(this.workspace, 'AUTOMAPPING', Workspace.Workspace.projectTypes.Network, 'simple website');
    if (workspace !== Workspace.Workspace.WorkspaceImpl.instance()) {
        new Persistence.FileSystemWorkspaceBinding.FileSystemWorkspaceBinding(Persistence.IsolatedFileSystemManager.IsolatedFileSystemManager.instance(), this.workspace);
    }
    this.failedBindingsCount = 0;
    this.automapping = new Persistence.Automapping.Automapping(this.workspace, this._onStatusAdded.bind(this), this._onStatusRemoved.bind(this));
...
AutomappingTest.prototype = {
    ...
    addNetworkResources: function (assets) {
        for (const url in assets) {
            ...
            const contentProvider = TextUtils.StaticContentProvider.StaticContentProvider.fromString(url, contentType, asset.content);
            const metadata = (typeof asset.content === 'string' || asset.time ?
...
//# sourceMappingURL=AutomappingTestRunner.js.map