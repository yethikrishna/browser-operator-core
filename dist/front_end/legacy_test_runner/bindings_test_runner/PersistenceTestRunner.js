...
export const waitForBinding = async function (fileName) {
    ...
    return TestRunner.waitForEvent(Persistence.Persistence.Events.BindingCreated, Persistence.Persistence.PersistenceImpl.instance(), binding => binding.network.name() === fileName || binding.fileSystem.name() === fileName);
};
...
class TestMapping {
    ...
    async addBinding(urlSuffix) {
        ...
        const networkUISourceCode = await TestRunner.waitForUISourceCode(urlSuffix, Workspace.Workspace.projectTypes.Network);
        const fileSystemUISourceCode = await TestRunner.waitForUISourceCode(urlSuffix, Workspace.Workspace.projectTypes.FileSystem);
...
//# sourceMappingURL=PersistenceTestRunner.js.map