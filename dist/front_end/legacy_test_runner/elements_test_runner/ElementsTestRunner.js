...
ElementsTestRunner.expandAndDumpEventListeners = function (eventListenersView, callback, force) {
    ...
    if (force) {
        ...
        TestRunner.addSniffer(EventListeners.EventListenersView.EventListenersView.prototype, 'eventListenersArrivedForTest', listenersArrived);
    }
};
/**
 * @param {!EventListeners.EventListenersView.EventListenersView} eventListenersView
...
ElementsTestRunner.waitForStyleCommitted = function (next) {
    TestRunner.addSniffer(Elements.StylePropertyTreeElement.StylePropertyTreeElement.prototype, 'editingCommitted', (...args) => {
...
ElementsTestRunner.waitForSelectorCommitted = function (callback) {
    TestRunner.addSniffer(Elements.StylePropertiesSection.StylePropertiesSection.prototype, 'editingSelectorCommittedForTest', callback);
};
ElementsTestRunner.waitForMediaTextCommitted = function (callback) {
    TestRunner.addSniffer(Elements.StylePropertiesSection.StylePropertiesSection.prototype, 'editingMediaTextCommittedForTest', callback);
};
ElementsTestRunner.waitForStyleApplied = function (callback) {
    TestRunner.addSniffer(Elements.StylePropertyTreeElement.StylePropertyTreeElement.prototype, 'styleTextAppliedForTest', callback);
};
...
ElementsTestRunner.dumpRenderedMatchedStyles = function () {
    ...
    function dumpRenderedProperty(property) {
        ...
        for (let i = 0; i < property.childCount(); ++i) {
            ...
            let text = indent;
            text += Platform.StringUtilities.sprintf('%s: %s', childProperty.nameElement.textContent, childProperty.valueElement.textContent);
...
async function printStyleSection(section, omitLonghands, includeSelectorGroupMarks) {
    ...
    const selector = section.titleElement.querySelector('.selector') || section.titleElement.querySelector('.keyframe-key');
...
ElementsTestRunner.dumpRule = function (rule, currentIndent) {
    ...
    if (rule.type === 'import') {
        TestRunner.addResult(currentIndent + '@import: header=' + ElementsTestRunner.rangeText(rule.headerRange) +
    ...
    TestRunner.addResult(currentIndent + '[UNKNOWN RULE]: header=' + ElementsTestRunner.rangeText(rule.headerRange) +
        ', body=' + ElementsTestRunner.rangeText(rule.bodyRange));
};
ElementsTestRunner.dumpStyle = function (style, currentIndent) {
    ...
    for (let i = 0; i < style.cssProperties.length; ++i) {
        ...
        if (!property.disabled) {
            TestRunner.addResult(currentIndent + '[\'' + property.name + '\':\'' + property.value + '\'' +
...
ElementsTestRunner.addNewRuleInStyleSheet = function (styleSheetHeader, selector, callback) {
    TestRunner.addSniffer(Elements.StylesSidebarPane.StylesSidebarPane.prototype, 'addBlankSection', onBlankSection.bind(null, selector, callback));
...
//# sourceMappingURL=ElementsTestRunner.js.map