...
function compareRuleRanges(lazyRule, originalRule) {
    ...
    for (let i = 0; i < flattenLazy.length; ++i) {
        ...
        if (!lazyRange.range.equal(originalRange.range)) {
            TestRunner.addResult('Error: ranges \'' + lazyRange.name + '\' are not equal: ' + lazyRange.range.toString() +
...
ElementsTestRunner.validateRuleRanges = function (selector, rules, callback) {
    ...
    function onContainerSelected() {
        ...
        if (fetchedRules.length !== rules.length) {
            TestRunner.addResult(Platform.StringUtilities.sprintf('Error: rules sizes are not equal! Expected: %d, actual: %d', fetchedRules.length, rules.length));
...
//# sourceMappingURL=StylesUpdateLinksTestRunner.js.map