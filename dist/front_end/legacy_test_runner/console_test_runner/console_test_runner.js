...
ConsoleTestRunner.dumpConsoleMessages = async function (printOriginatingCommand, dumpClassNames, formatter) {
    TestRunner.addResults(await ConsoleTestRunner.dumpConsoleMessagesIntoArray(printOriginatingCommand, dumpClassNames, formatter));
};
/**
 * @param {boolean=} printOriginatingCommand
 * @param {boolean=} dumpClassNames
...
ConsoleTestRunner.dumpConsoleMessagesIgnoreErrorStackFrames =
    async function (printOriginatingCommand, dumpClassNames, messageFormatter) {
        TestRunner.addResults(await ConsoleTestRunner.dumpConsoleMessagesIntoArray(printOriginatingCommand, dumpClassNames, ConsoleTestRunner.formatterIgnoreStackFrameUrls.bind(this, messageFormatter)));
    };
        ...
        for (let i = 0; i < messageViews.length; ++i) {
            ...
            for (let node = element; node; node = node.traverseNextNode(element)) {
                ...
                for (let j = 0; j < treeElements.length; ++j) {
                    for (let treeElement = treeElements[j]; treeElement; treeElement = treeElement.traverseNextTreeElement(true, null, true)) {
...
ConsoleTestRunner.expandGettersInConsoleMessages = function (callback) {
    ...
    TestRunner.addSniffer(ObjectUI.ObjectPropertiesSection.ObjectPropertyTreeElement.prototype, 'updateExpandable', propertyExpandableUpdated);
    for (let i = 0; i < messageViews.length; ++i) {
        const element = messageViews[i].element();
        for (let node = element; node; node = node.traverseNextNode(element)) {
...
ConsoleTestRunner.waitUntilConsoleEditorLoaded = function () {
    ...
    if (prompt.editor) {
        ...
        TestRunner.addSniffer(Console.ConsolePrompt.ConsolePrompt.prototype, 'editorSetForTest', _ => fulfill(prompt.editor));
    }
    return promise;
};
/**
...
ConsoleTestRunner.dumpStackTraces = function () {
    ...
    for (let i = 0; i < viewMessages.length; ++i) {
        ...
        TestRunner.addResult('Message[' + i + ']: ' + Bindings.ResourceUtils.displayNameForURL(m.url || '') + ':' + m.line + ' ' +
            m.messageText);
...
//# sourceMappingURL=console_test_runner.js.map