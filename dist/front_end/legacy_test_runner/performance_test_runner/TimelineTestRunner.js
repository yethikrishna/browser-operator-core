...
PerformanceTestRunner.createTimelineController = function () {
    const controller = new Timeline.TimelineController.TimelineController(SDK.TargetManager.TargetManager.instance().primaryPageTarget(), Timeline.TimelinePanel.TimelinePanel.instance());
...
PerformanceTestRunner.dumpFlameChartProvider = function (provider, includeGroups) {
    ...
    for (let groupIndex = 0; groupIndex < timelineData.groups.length; ++groupIndex) {
        ...
        const maxLevel = (groupIndex + 1 < timelineData.groups.length ? timelineData.groups[groupIndex + 1].startLevel : stackDepth);
...
`);
//# sourceMappingURL=TimelineTestRunner.js.map