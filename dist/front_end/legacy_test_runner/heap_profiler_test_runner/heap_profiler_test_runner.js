...
HeapProfilerTestRunner.createHeapSnapshotMockFactories = function () {
    ...
    HeapProfilerTestRunner.createHeapSnapshot = function (instanceCount, firstId) {
        ...
        const gcRootsNode = new HeapProfilerTestRunner.HeapNode('(GC roots)', 0, HeapProfilerTestRunner.HeapNode.Type.synthetic);
...
HeapProfilerTestRunner.startProfilerTest = function (callback) {
    ...
    TestRunner.addSniffer(Profiler.HeapProfilerPanel.HeapProfilerPanel.instance(), 'addProfileHeader', HeapProfilerTestRunner.profileHeaderAdded, true);
    TestRunner.addSniffer(Profiler.ProfileView.ProfileView.prototype, 'refresh', HeapProfilerTestRunner.profileViewRefresh, true);
    TestRunner.addSniffer(Profiler.HeapSnapshotView.HeapSnapshotView.prototype, 'show', HeapProfilerTestRunner.snapshotViewShown, true);
...
HeapProfilerTestRunner.checkArrayIsSorted = function (contents, sortType, sortOrder) {
    ...
    for (let i = 0; i < contents.length - 1; ++i) {
        ...
        if (result !== 0 && result !== acceptableComparisonResult) {
            TestRunner.addResult('Elements ' + i + ' and ' + (i + 1) + ' are out of order: ' + a + ' ' + b + ' (' + sortOrder + ')');
        }
    }
};
HeapProfilerTestRunner.clickColumn = function (column, callback) {
    ...
    function sortingComplete() {
        HeapProfilerTestRunner.currentGrid().removeEventListener(Profiler.HeapSnapshotDataGrids.HeapSnapshotSortableDataGridEvents.SortingComplete, sortingComplete, this);
    ...
    HeapProfilerTestRunner.currentGrid().addEventListener(Profiler.HeapSnapshotDataGrids.HeapSnapshotSortableDataGridEvents.SortingComplete, sortingComplete, this);
...
HeapProfilerTestRunner.takeSnapshotPromise = function () {
    return new Promise(resolve => {
        ...
        function finishHeapSnapshot() {
            ...
            dataGrid.addEventListener(Profiler.HeapSnapshotDataGrids.HeapSnapshotSortableDataGridEvents.SortingComplete, sortingComplete, null);
            function sortingComplete() {
                dataGrid.removeEventListener(Profiler.HeapSnapshotDataGrids.HeapSnapshotSortableDataGridEvents.SortingComplete, sortingComplete, null);
                resolve(profile);
            }
        }
    });
...
//# sourceMappingURL=heap_profiler_test_runner.js.map