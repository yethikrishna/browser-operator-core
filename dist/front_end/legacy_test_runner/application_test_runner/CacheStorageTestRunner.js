...
export const dumpCacheTree = async function (pathFilter) {
    ...
    const promise = TestRunner.addSnifferPromise(SDK.ServiceWorkerCacheModel.ServiceWorkerCacheModel.prototype, 'updateCacheNames');
...
export const deleteCacheFromInspector = async function (cacheName, optionalEntry) {
    ...
    const cachesTreeElement = Application.ResourcesPanel.ResourcesPanel.instance().sidebar.cacheStorageListTreeElement;
    let promise = TestRunner.addSnifferPromise(SDK.ServiceWorkerCacheModel.ServiceWorkerCacheModel.prototype, 'updateCacheNames');
    Application.ResourcesPanel.ResourcesPanel.instance().sidebar.cacheStorageListTreeElement.refreshCaches();
    await promise;
    if (!cachesTreeElement.childCount()) {
    ...
    for (let i = 0; i < cachesTreeElement.childCount(); i++) {
        ...
        promise = TestRunner.addSnifferPromise(Application.ServiceWorkerCacheViews.ServiceWorkerCacheView.prototype, 'updateDataCallback');
        let view = cacheTreeElement.view;
        if (!view) {
            cacheTreeElement.onselect(false);
        }
...
export const waitForCacheRefresh = function (callback) {
    TestRunner.addSniffer(SDK.ServiceWorkerCacheModel.ServiceWorkerCacheModel.prototype, 'updateCacheNames', callback, false);
};
...
`);
//# sourceMappingURL=CacheStorageTestRunner.js.map